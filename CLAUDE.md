...
현재 버전: v5.16 (단일 HTML 파일: `music-church-maker.html`)

## v5.16 변경 (2026-07-07)
- **PPT 자동 줄바꿈 금지**: 가사·제목 addText에 `wrap:false` (pptx XML `bodyPr wrap="none"` 확인). 원인: 앱의 글자폭 어림값(한글 1.02em)보다 실제 폰트(맑은 고딕 bold)가 넓으면 파워포인트가 상자 폭에서 줄을 재분할해 미리보기와 달라짐. 이제 줄 구조가 미리보기와 동일, 살짝 긴 줄은 좌우 여백(7%)을 침범(빔 안전영역 내).
- v5.15: GCG 미리보기 안내(글꼴은 근사치) 문구 추가.
